...
*** Count unique elements
This algorithm counts the number of unique elements in an integer array by first sorting the array and then iterating through it to count distinct elements.
...
*** Occurrence count
This algorithm counts the number of occurrences of a target element in an integer array.
...
** Sorting
*** Count sort
Count sort is a non-comparative sorting algorithm that sorts an array by counting the occurrences of each unique element. The steps are:
...
*** Heap sort
Heap sort is a comparison-based sorting technique based on a binary heap data structure. It sorts an array by:
...
*** Insertion sort
Insertion Sort is a simple sorting algorithm that builds the final sorted array one item at a time. It is much less efficient on large lists than more advanced algorithms such as quicksort, heapsort, or merge sort. The key steps are:
