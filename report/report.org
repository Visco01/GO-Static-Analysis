...
The results from LiSA will be investigated to identify potential
...
#+LATEX: \newpage
* Domains involved into the analyses
...
- `str`: The input string remains unchanged.
...
- `count`: The variable to store the occurrence count, starting from 0 and increasing.
...
**** Prefix domain analysis
- `str`: The input string remains unchanged.
- `length`: The length of the string, which is fixed.
...
- `count`: The length of the `words` slice.
**** Suffix domain analysis
- `str`: The input string remains unchanged.
...
- `count`: Accumulates the count of occurrences of `c`.
...
#+LATEX: \newpage
* Considerations about LiSA
Below I want to discuss about some features that I personally appreciate and possible improvements for the future developments of LiSA.

First and most importantly, LiSA is built on a strong *theoretical foundation*,
leveraging abstract interpretation principles to ensure that analyses are both
sound and effective. Another benefit is that LiSA's design as a library allows
users to easily extend and customize the framework to suit specific analysis
needs such as the support of new programming languages.

In addition, there are some aspects that could be enhanced in the future develompents:
- **Optimization for Large-Scale Analyses**: Addressing performance bottlenecks,
  particularly in the context of large-scale codebases, could enhance LiSA's
  applicability in industrial settings.
- **Enhanced User Interface and Visualization**: Developing a more user-friendly
  interface, including better visualization tools for analysis results, could
  make LiSA more accessible to a broader audience. This would help users better
  understand the outcomes of their analyses and more easily interpret complex
  results.

  Personally, I would suggest the following improvements:
  1) Inside the html report, I would add a function that "stretches" the graph
     to make it more readable. In addition, I would add a label on each node
     to better understand the sequential order of the code (for example a
     numerical label: 1, 2, 3, etc... for each node of the graph).
  2) When I had to deal with algorithms of a certain size, I had some troubles while analyzing the graph. One way to simplify this process could be making the two sidebars collapsible (or at least resizable) to give the user the ability to fully focus on the graph structure.
- **Broader Language Support**: While LiSA is designed to be adaptable to
  various programming languages, expanding its support for more
  languages could make it even more versatile and appealing to a wider range of
  developers and researchers.

#+LATEX: \newpage
* Conclusions
This project has provided an exploration of various algorithms, focusing on
their analysis through different domains: taint analysis, signs, intervals,
prefix, suffix, parity. The approach aimed to dissect each algorithm's behavior,
understanding how data flows through them and how different aspects of the data
(such as taint or interval properties) affect the outcomes.

Throughout the project, I focused on understanding how data correctness is
maintained in algorithms. For example, taint analysis was a critical aspect
where we evaluated whether data could be considered safe (untainted) or
potentially dangerous (tainted). This was particularly relevant in algorithms
where data could originate from external inputs (the algorithms provided are
simple and there are not any calls to external resources into them.
Nevertheless, in a broader context with a real-world scenario, taint analysis is
one of the most useful), as this raised concerns about security and correctness.

The analysis also covered how algorithms handle edge cases, such as empty inputs
or extreme values, and how they maintain their integrity across different input
conditions. This helped to ensure that the algorithms are robust and reliable in
a variety of scenarios.

In summary, this project not only provided a detailed technical analysis of each
algorithm but also ensured that the broader implications of data handling,
safety, and security were considered. The project underscores the importance of
thorough and multidomain analysis in algorithm design and evaluation.
